...
const CATEGORIES = [
  // DEV EFFICIENCY categories first (checked before broad AI categories)
  {
    ...
    keywords: [
      ...
      // Android 逆向工程 / Reverse Engineering
      "android-reverse", "android-re", "reverse-engineering",
      "frida", "frida-android", "android-frida", "objection",
      "jadx", "dex2jar", "bytecode-viewer", "smali", "baksmali",
      "dex", "dex-patcher", "apk-patcher", "apk-mod",
      "xposed", "lsposed", "edxposed", "xposed-module", "xposed-framework",
      "android-hook", "android-hooking", "hook-framework",
      "native-hook", "so-inject", "so-hook", "elf-hook",
      "android-unpacker", "apk-unpack", "unpacker",
      "ghidra", "radare2", "binary-ninja", "ida-pro",
      "android-ssl-pinning", "ssl-unpin", "certificate-pinning",
      "android-pentest", "android-pentesting", "android-security",
      "lspatch", "android-patch", "android-crack",
      "r0capture", "androguard", "mobsf", "mobile-security-framework",
      "appshark", "drozer", "android-tester",
      "oat", "art", "dalvik", "elf", "android-native",
      "apk-sign", "apk-resign", "apk-analyzer",
      "virtual-app", "virtualapk", "sandvxposed", "taichi"
    ]
  },
  {
    name: "🧪 自动化测试 / 质量保障",
...
async function fetchTrending() {
  ...
  // ===== 2.5 Android Reverse Engineering =====
  const androidReQs = [
    `topic:frida stars:>10`,
    `topic:xposed stars:>10`,
    `jadx OR apktool OR smali OR dex2jar in:name,description stars:>20`,
    `android hook in:name,description stars:>20`,
    `android reverse engineering in:name,description stars:>10`,
    `lsposed OR magisk in:name,description stars:>30`,
    `android pentest in:name,description stars:>5`,
    `android ssl pinning in:name,description stars:>5`,
    `mobile security framework in:name,description stars:>5`,
    `apk tool android in:name,description stars:>10`,
    `androguard OR mobsf OR drozer in:name,description stars:>5`,
  ];
  for (const q of androidReQs) {
    const data = await ghRequest(makePath(q, 8));
    totalCalls++;
    if (data.items) results.push(...data.items);
    console.log(`  [${totalCalls}] android-re: "${q.slice(0,50)}" -> ${(data.items||[]).length} items`);
    await new Promise((r) => setTimeout(r, DELAY));
  }
